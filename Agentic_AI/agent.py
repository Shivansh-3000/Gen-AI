...
- run_cmd(cmd: str): Takes a system linux command as string ans executes the command on users system and retuens the output from that command.
...
while True:
    ...
    while True:
        ...
        if parsed_result.step == "TOOL":
            ...
            weather_result = get_weather(tool_input)

            message_history.append(
                {
                    "role": "developer",
                    "content": json.dumps({"step":"OBSERVE", "tool":tool_to_call, "input":tool_input, "output": tool_response})
                }
            )
            continue
